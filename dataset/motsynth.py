...
from utils.transforms import (ComposeBB, ConvertBCHWtoCBHW, ConvertBHWCtoBCHW,
                              ConvertImageDtypeBB, NoisyBB, RandomCrop,
                              RandomHorizontalFlip, Resize, ToTensorBB)
from utils.utils_scripts import Timer, z_to_nearness
from torchvision.transforms.functional import InterpolationMode


class MOTSynth(VideoFrameDataset):
    def __init__(self, cnf: argparse.Namespace, mode: str = 'train', return_only_clean: bool = False, seq=None, no_transforms: bool = False) -> None:
        super().__init__(cnf)
        self.FILENAME_LEN = 4
        self.MODE = mode
    ...
    def __getitem__(self, idx: int) -> Tuple[np.ndarray, list]:
        ...
        if self.return_only_clean:
            ...
            if self.MODE == 'train':
                crop = transforms.RandomResizedCrop(self.input_size, scale=(0.2, 1.0), interpolation=InterpolationMode.BICUBIC)
            else:
                crop = transforms.Compose([transforms.Resize(self.input_size[0], interpolation=InterpolationMode.BICUBIC),
        ...
        c_maps = torch.tensor([])
        d_map = torch.zeros((1, int(self.input_size[0] * self.cnf.scale_factor),
                             int(self.input_size[1] * self.cnf.scale_factor)))
        m_map = torch.zeros((1, int(self.input_size[0] * self.cnf.scale_factor),
                             int(self.input_size[1] * self.cnf.scale_factor)))
        ...
        for i, img in enumerate(clip):
            ...
            c_map, _, _, d_map_np, m_map_np = gt.get_gt(
                img=img, bboxes=video_bboxes[i], dists=d, sigma=self.cnf.sigma,
                scale_factor=self.cnf.scale_factor, ds_stats=self.cnf.ds_stats,
                radius=self.cnf.radius
            )

            if self.cnf.use_centers:
                c_maps = torch.cat((c_maps, torch.tensor(c_map, dtype=torch.float).unsqueeze(0)), dim=0)
    ...
    def _get_labels(self, video_name, frames_name):
        ...
        return [labels[(labels[:, 0] == float(frame))
                       & (labels[:, 5] == 1.0)
    ...
    def get_transforms(cnf, mode):
        if mode == 'train':
            ...
            if cnf.random_crop:
                ...
                T_BB.transforms.append(RandomCrop(cnf.input_h_w, crop_mode=cnf.crop_mode,
            ...
            T_CLIP = transforms.Compose([
                transforms.RandomApply([
                    transforms.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.4, hue=0.1)], p=0.25),
                transforms.RandomApply([
                    transforms.GaussianBlur(kernel_size=cnf.aug_gaussian_blur_kernel,
                                            sigma=cnf.aug_gaussian_blur_sigma)], p=0.25),
            ...
            T_CLIP = transforms.Compose([ConvertBCHWtoCBHW(), ])
        return T_BB, T_CLIP
    ...
    def collate_fn(batch):
        ...
        return torch.stack(clip), torch.stack(d_map), torch.stack(m_map), *other, torch.stack(clip_clean)
