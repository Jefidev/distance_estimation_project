...
def vit_large(patch_size=14, **kwargs):
    model = VisionTransformer(
        img_size=[518], patch_size=patch_size, embed_dim=1024, depth=24, num_heads=16, mlp_ratio=4,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model
...
class ViTFeat(nn.Module):
    ...
    def __init__(self, args, pretrained_pth="https://dl.fbaipublicfiles.com/dino/dino_vitbase8_pretrain/dino_vitbase8_pretrain.pth", feat_dim=768, vit_arch = 'base', vit_feat = 'k', patch_size=8):
        ...
        if vit_arch == 'base' :
            ...
            self.model = vit_large(patch_size=patch_size, num_classes=0)
